...
#combine all calibration curve outputs and 
cal.curve.output <- left_join(lm.out.slopes, lm.out.intercepts) %>%
  mutate(std_error = sqrt(((std.error.slope/slope)^2)+((std.error.intercept/intercept)^2)))
...
blk.plot.1
#Fix mislabeled RC104 homarine blanks, rearange as follows:
# (3000nM -> 1000nM)
# (1000nM -> 2000nM)
# (2000nM -> 3000nM)
...
dat.q.m <- dat.q.2 %>%
  ...
  left_join(., mdat.tidy) %>%
  mutate(nM_in_smp = nM.in.vial.adj*(400*1e-6)/Vol_Filt_L,
         nM.per.hour.1 = nM_in_smp/(incubation_time1/60),
         nM.per.hour.2 = nM_in_smp/(incubation_time2/60))
